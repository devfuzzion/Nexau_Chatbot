...
export const logConversation = async ({ userId, question, response, threadId, messageId }) => {
  try {
...
export const logFeedback = async ({ userId, feedback, threadId, messageId }) => {
  try {
...
export const storeFeedbackState = async ({ userId, messageId, threadId, isLiked }) => {
  try {
    ...
    if (existingRecords.length > 0) {
      ...
      const record = await base(FEEDBACKS_TABLE_NAME).update(existingRecords[0].id, {
